...
import os
import json
from .retriever import BaseFaissIPRetriever, BaseFaissHNSWRetriever
...
def main():
    ...
    parser.add_argument('--save_index_path')
    ...
    if args.use_hnsw:
        ...
        retriever = BaseFaissHNSWRetriever(p_reps_0)

        if os.path.exists(os.path.join(args.save_index_path, "index.faiss")):
            logger.info("Loading from saved index")
            retriever.load(os.path.join(args.save_index_path, "index.faiss"))
            look_up = json.load(open(os.path.join(args.save_index_path, "index.json")))
            ...
            for p_reps, p_lookup in shards:
                all_p_reps.extend(p_reps)
                look_up += p_lookup
            
            all_p_reps = np.array(all_p_reps, dtype=np.float32)
            logger.info("Building index")
            retriever.build(all_p_reps)
            logger.info("Saving index")
            retriever.save(os.path.join(args.save_index_path, "index.faiss"))
            json.dump(look_up, open(os.path.join(args.save_index_path, "index.json"), "w"))
